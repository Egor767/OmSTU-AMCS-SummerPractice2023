...
public class Spacebattle
{
    ...
    public Ship Move(Ship ship)
    {
        ...
        //Проверка угла и топлива
        if (ship.angle == double.NaN || ship.turn == double.NaN)
        {
            throw new System.Exception();
        }
        
        if (ship.turn_possible == false)
        {
            throw new System.Exception();
        }

        //Изменение значений
        if (Math.Abs(ship.fuel_reserve - ship.waste_fuel)>=eps)
        {
            ship.fuel_reserve -= ship.waste_fuel;
        }
        else
        {
            throw new System.Exception();
        ...
        ship.angle = ship.angle + ship.turn;
